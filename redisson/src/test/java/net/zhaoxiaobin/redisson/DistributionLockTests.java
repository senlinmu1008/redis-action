...
//        long successCount = IntStream.range(0, 500).parallel().filter(j -> distributionLockDemo.increment(j)).count();

        System.out.println(successCount); // 成功数
        System.out.println(distributionLockDemo.get()); // 剩余库存
    }
}